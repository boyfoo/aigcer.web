...
- 拉片页结合整片总览与单镜头细读：默认显示看点与镜头列表，镜头参考链接直接进入细读，两种视角共用播放器。镜头列表只保留阅读区总览这一套，不再在播放器下重复展示横向缩略图列表；细读通过上一镜、下一镜和全部镜头导航。总览支持首帧/代表画面与可选尾帧放大、列表/网格、片内搜索、景别/运镜筛选和镜长排序；排序后保留原镜号。返回总览保留筛选及浏览位置，保留原有分析、提示词复制、收藏与项目参考集。
- 整片节奏带按真实镜长占比显示，未拆解时间保留空段。跟随播放开关默认关闭以保持正在阅读的镜头，开启后随播放位置切换分析。不得通过展开多份长内容实现镜头切换，或增加独立的纵向滚动区。
- 逐镜头支持本地上传可选尾帧，以及可选的声音与音乐、台词、画面文字、叙事作用。未填写尾帧不生成替代帧，也不在访客阅读页留空框；首帧使用案例封面兜底时明确标注。真实内容不附带模拟标注或自动推测的分析。新增资料沿用原有保存草稿、手动发布快照和下架/重新上架流程。
- 访客浏览优先级为“看案例 → 看成片 → 读镜头解释 → 按需看参数/提示词 → 收藏”。全站主导航为“看案例、我的收藏”，视频、分镜图片和提示词参考放在搜索附近的分类导航；保留原有可抓取网址。首页保留精选大图与辅助图片，卡片名称与看点常显，分类/搜索结果采用便于比较的常规卡片列表。
- 分类分组默认收起，在原位置展开筛选；手机端同样保留页面内分类，不增加弹窗。选中条件与结果数量可见，并可清除条件。首页看点来自已有分析或标签，不生成未经录入的学习结论。
- 案例页播放器上方不展示“1 看成片”和“先看完整视频，再选一个镜头读拆解。”，不保留该引导区的占位与留白；单镜头细读按“标题与切换操作 → 当前镜头画面 → 概述与拆解正文 → 折叠的参数和提示词”排列。当前镜头画面常显，默认首帧/代表画面；有尾帧时在同一位置切换，画面区域高度固定，点击沿用放大查看，切换镜头后回到首帧。桌面保持左侧视频、右侧阅读，统计、人物及质量报告也在右侧继续排列，使左侧播放区在整个阅读过程中保持可见。窄屏播放器以较小高度吸附顶部，镜头切换和报告跳转为播放器预留顶部空间；正文前仍显示当前镜头画面。“拆解跟随播放”控制右侧镜头选择，开关与开启/关闭说明常显在拆解区顶部，不收进左侧工具。节奏与标注收在“播放与研究工具”；镜头搜索、筛选和排列就地展开。画面参数、术语解释及提示词在正文之后按需查看。提供“回看这镜”，切换下一镜保持阅读位置，不增加独立的纵向滚动区。
- 案例页底部不展示“对比其他镜头”或其他案例推荐区域。
- “镜头拆解、整片统计、出场人物、质量检查、导出报告”这排内容导航只放在右侧阅读区顶部并吸附，按“内容导航 → 拆解跟随播放 → 当前内容”排列；保留原有报告跳转，窄屏在阅读区内自然换行并吸附在播放区下方，不再横跨左右两栏。报告跳转按导航实际高度预留空间，工具展开/收起或导航换行后同步更新，避免遮住标题。
- 视频、播放状态与“播放与研究工具”作为一个整体吸附，工具默认展开且可手动收起，入口始终可见；展开时按工具实际高度缩小视频，保持原视频比例和画面标注对齐。桌面展开/收起不改变右侧阅读位置；窄屏整组最多约占可视高度的 60%，镜头和报告跳转按整组实际高度预留空间，不增加独立滚动条。
- 播放区进入吸附状态时，在左侧播放器上方固定显示仅含返回按钮和案例名称的精简标题，不带类型、时长、镜头数量或收藏操作；滚回上方时隐藏精简标题并保留原始完整标题。精简标题在播放组上方独立占位，不参与视频的高度分配，播放器在固定前后保持相同尺寸；不推动右侧阅读位置，窄屏导航和报告跳转同时预留精简标题与播放组的高度。长名称单行省略，返回沿用视频案例列表入口。
- 精简标题通过 180ms 淡入和 6px 上移出现，140ms 退出，支持中途反向滚动时平滑反转；仅改变透明度与位移，不缩放视频或改变正文布局。隐藏时不可聚焦或点击；键盘操作和系统“减少动态效果”模式直接切换。
- 拉片报告补齐整片统计（平均/中位/最短/最长镜长、相邻边界切换频率）、按时长计算的景别/类别/运镜/叙事节奏分布、人物索引与关联筛选、15 项质量检查、JSON 与离线 HTML 导出，以及视频宽高/画幅/帧率/音轨展示。整片统计、出场人物、质量检查、导出报告默认展开，仍可手动收起；固定导航根据当前阅读区块显示黄色选中态，点击可跳转，滚动、区块高度变化和到达页底时同步更新。继续保留根页面滚动。
- 录入页可管理人物图片和介绍，在镜头中就地新增并关联人物、选择类别/节奏/转场，并填写对照原片的人工复核记录；改动镜头资料或替换视频后清除旧的复核确认。视频宽高随上传读取，未知帧率/音轨允许手工补充，不推测。所有新资料沿用草稿、发布快照、下架/重新上架的保存方式。
- 质量报告区分通过、需处理、待复核：结构校验可以自动通过，画面/运镜/切点/类别依据/叙事判断须有作者复核结论；示例数据不得显示为已与原片核对。空数据、缺帧、未填写和无检测证据不等于通过。分布保留“未填写”和“未拆解时段”。
- 离线报告包含图片、镜头资料、检索与排序、人物筛选、统计及检查；原视频由阅读者在离线页选择本地文件，不上传。图片读取失败或达到导出大小限制时明确注明缺图。人物图片也必须纳入 Sites 已发布快照素材导出。
- 对访客统一使用“收藏案例”和“镜头收藏夹”：前者保存整条案例，后者整理不同案例中的具体镜头。原项目参考集在界面中称为镜头收藏夹，保留原存储键、分组、备注和排序数据。“收藏这镜”作为次要操作，不抢占阅读主动作。
