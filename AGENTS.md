...
- 拉片页结合整片总览与单镜头细读：默认显示看点与镜头列表，镜头参考链接直接进入细读，两种视角共用播放器。总览支持首帧/代表画面与可选尾帧放大、列表/网格、片内搜索、景别/运镜筛选和镜长排序；排序后保留原镜号。返回总览保留筛选及浏览位置，保留原有分析、提示词复制、收藏与项目参考集。
...
- 案例页以“看成片”和“看镜头拆解”明确顺序，进入单镜头后直接显示解释正文。“拆解跟随播放”控制右侧镜头选择，开关与开启/关闭说明常显在拆解区顶部，不收进左侧工具。节奏、标注与缩略图收在“播放与研究工具”；镜头搜索、筛选和排列就地展开。画面参数、术语解释及提示词在正文之后按需查看。手机阅读时视频不遮住正文，提供“回看这镜”，切换下一镜保持阅读位置。
- 案例页底部不展示“对比其他镜头”或其他案例推荐区域。
- 拉片报告补齐整片统计（平均/中位/最短/最长镜长、相邻边界切换频率）、按时长计算的景别/类别/运镜/叙事节奏分布、人物索引与关联筛选、15 项质量检查、JSON 与离线 HTML 导出，以及视频宽高/画幅/帧率/音轨展示。入口清楚可见，详细报告按需展开，继续保留根页面滚动。
