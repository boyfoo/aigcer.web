...
export function ReferenceProjectsProvider({ children }) {
  ...
  const commit = (actions) => {
    if (!readyRef.current) { message.error("参考集尚未读取完成，请稍后重试"); return false; }
    try {
      ...
      message.error(error.name === "QuotaExceededError" ? "浏览器存储空间不足，本次修改未保存" : error.message || "无法保存参考集，请检查浏览器存储权限");
      return false;
    }
  };
...
function AddReferenceDialog({ reference, preferredId, onClose, onAdded }) {
  ...
  const save = () => {
    ...
    if (commit(actions)) { onAdded(id); message.success("镜头已加入项目参考集"); onClose(); }
  };
  return <Modal className="reference-modal" open title="加入项目参考集" onCancel={onClose} onOk={save} okText={exists ? "已在项目中" : "加入参考集"} okButtonProps={{ disabled: !ready || !shot || exists || (projectId === "new" && !name.trim()) }} cancelText="取消">
    ...
    <label className="reference-field">项目<Select aria-label="选择参考项目" value={projectId} onChange={(value) => { setProjectId(value); setGroupId(""); }} options={[...projects.map(({ id, name: label }) => ({ value: id, label })), { value: "new", label: "＋ 新建项目" }]} /></label>
    {projectId === "new" ? <label className="reference-field">项目名称<Input autoFocus aria-label="新项目名称" placeholder="例如：雨夜品牌短片" value={name} maxLength={60} onChange={(event) => setName(event.target.value)} onPressEnter={save} /></label> : <label className="reference-field">分组<Select aria-label="选择镜头分组" value={groupId} onChange={setGroupId} options={[{ value: "", label: "未分组" }, ...(project?.groups ?? []).map(({ id, name: label }) => ({ value: id, label }))]} /></label>}
    <label className="reference-field">参考备注<Input.TextArea aria-label="镜头参考备注" placeholder="记录这个镜头对项目有什么帮助" value={note} onChange={(event) => setNote(event.target.value)} maxLength={2000} autoSize={{ minRows: 3, maxRows: 5 }} /></label>
    <p className="reference-storage-note">参考集保存在当前浏览器。</p>
  </Modal>;
}
...
function ProjectLibrary({ open, onClose, activeId, setActiveId, loadError, guardNavigation }) {
  ...
  return <>
    <Drawer title="项目参考集" open={open} onClose={onClose} size={1000} className="reference-library" extra={<span className="reference-storage-note">保存在当前浏览器</span>}>
      {loadError ? <Alert type="error" title="暂时无法读取参考集" description="本地数据未被覆盖。请检查浏览器存储权限，或恢复数据后刷新页面。" /> : !ready ? <p>正在读取参考集…</p> : <div className="reference-library-layout">
        <aside className="reference-project-list" aria-label="参考项目">
          <div className="reference-new-project"><Input aria-label="创建项目名称" placeholder="新项目名称" value={newProjectName} onChange={(event) => setNewProjectName(event.target.value)} maxLength={60} onPressEnter={createProject} /><Button icon={<PlusOutlined />} aria-label="创建项目" onClick={createProject} disabled={!newProjectName.trim()} /></div>
        ...
        <div className="reference-project-content">
          {!project ? <Empty description="为下一个项目，整理你的第一组镜头参考" /> : <>
            <header className="reference-project-heading"><div><h2>{project.name}</h2><p>{project.references.length} 个镜头 · {project.groups.length} 个分组</p></div><div><Button type="text" icon={<EditOutlined />} aria-label="重命名项目" onClick={() => setEditingName({ kind: "project", id: project.id, name: project.name })} /><Popconfirm title="删除这个项目？" description="只删除参考集，不会删除视频案例。" okText="删除" cancelText="取消" onConfirm={() => commit({ type: "removeProject", projectId: project.id })}><Button type="text" danger icon={<DeleteOutlined />} aria-label="删除项目" /></Popconfirm></div></header>
            ...
            {!project.references.length && <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="在视频页选择镜头，点击“加入项目”" />}
            {groups.map((group) => {
    ...
    <Modal className="reference-modal" title={editingName?.kind === "project" ? "重命名项目" : "重命名分组"} open={!!editingName} onCancel={() => setEditingName(null)} okText="保存" cancelText="取消" onOk={() => {
